...
{
    /**
     * Take in a phrase that has many words separated by spaces. Return the longest word that
     * you can find within the phrase -  don't forget, hyphenated words are allowed!
    ...
    public static String longestWord(String aPhrase)
    {
        ...
        /*String[] phraseArr = aPhrase.split(" ");
        String longestString = "";
        
        for(int i = 0; i < phraseArr.length; i++)
        {
            if(phraseArr[i].length() > longestString.length())
            {
                longestString = phraseArr[i];
            }
        }
